...
from CETools.windows.rendering.rdConstants import *
...
class MainWindow(MayaQWidgetDockableMixin, QtWidgets.QDialog):
    ...
    def build_win(self):
        self.setWindowTitle("CE Rendering Toolkit")
        ...
        # SHELVES
        overscan_box = GroupBox(name="Overscan".upper(), is_open=True)
        overscan_layout = QtWidgets.QGridLayout()
        overscan_box.vbox.addLayout(overscan_layout)
        overscan_widget = rdt.OverscanWidget(parent=self, tool_layout=overscan_layout,
                                             dir_path=scene_directory)
        overscan_widget.hide()
        ...
        overscan_shelf_btn = ToolkitButton(icon_path=OVERSCAN_ICON_CONST.format(self.filePath),
                                           tool_tip=OVERSCAN_DESC_CONST)
        overscan_shelf_btn.left_clicked.connect(
            lambda: toggle_visibility(layout=page_layout, button=overscan_shelf_btn, widget=overscan_box))
        overscan_shelf_btn.double_clicked.connect(
            lambda: rdt.ToolWindow(tool="Overscan", parent=self, window=True, width=OVERSCAN_WIDTH_CONST,
                                   height=OVERSCAN_HEIGHT_CONST))
        tool_layout.addWidget(overscan_shelf_btn)
        overscan_shelf_btn.setStyleSheet("""           
                QPushButton:hover {{
